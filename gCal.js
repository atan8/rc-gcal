...
function authorize(credentials, myRes, myCallBack, callback) {
  ...
  fs.readFile(TOKEN_PATH, (err, token) => {
    if (err) return getAccessToken(oAuth2Client, myRes, myCallBack, callback);
    oAuth2Client.setCredentials(JSON.parse(token));
    callback(oAuth2Client, myRes, myCallBack);
  });
}
 ...
 */
function getAccessToken(oAuth2Client, myRes, myCallBack, callback) {
